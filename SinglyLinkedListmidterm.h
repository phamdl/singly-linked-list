...
public:
  ...
  //This throws the exception if the size is zero
  else if (Size() == 1){
    return head->Element();
  }
 return head->Element() - Difference(head->Next());
}
//Takes the elements in the list and returns the difference
//Essentially taking the first value and subtracting the following values
//Returns a logic exception error if no elements are found, aka the Size() is equal to 0
//Returns the single element if only one number is present, aka if size() is equal to 1
  void Clear() {
    while (size_ > 0) {
      RemoveFront();
    }
...
private:
  ...
  int size_;
T Difference(std::shared_ptr<SLNode<T>> head){
  if (!=head){
    return 0;
  }
  return head->Element() + Difference(head->Next());
